...
<input type="submit" name="hapus_toko" value="Status : in Trouble" style="background: red;color: white;font-size: 24px;width: 100%;height: 50px;"><?php if (empty($_POST['info_resesi'])) {
 ?><br><br><H3>Latar Belakang Masalah :</H3><?php if (!empty($status_detail)&&$status_toko=="in Trouble") {
     ?> <textarea name="info_resesi" value="<?php echo $status_detail; ?>" rows="6" cols="40"></textarea> <?php 
 } else { ?><textarea name="info_resesi" placeholder="contoh : (masalah internal toko (masalah keuangan), perlu reksadana, perlu inkubator bisnis (diskusi dengan pegiat bisnis digital), tambah fitur a karena.., kurangi fitur b karena..., bug di bagian order, desain sulit dipahami, dll.)" rows="6" cols="40"></textarea><?php } ?><br><br><b>- Wajib sertakan kalimat didalam kurung, misal (masalah b....)<br>- Fitur ini akan menonaktifkan akun toko dari panel penjualan</b><br><br><h3 style="color : red;">- 1 pesan = membantu anda maju menuju ukm digital</h3><br><br><input type="submit" value="set" name="resesi" class="btn_submit">&nbsp;<input type="submit" value="Batal" name="batal_proses" class="btn_submit"></form></center></div><div class="cleared"></div><br><br> <?php } else {
 ...
 ?><br><br><H3>Set Jam Buka :</H3><?php if (!empty($status_detail)&&$status_toko=="in Trouble") {
    ...
    $status="Active";$info=$_POST['info_open']; @mysqli_query($conn,"UPDATE infotoko SET status='$status',info_status='$info' WHERE id_toko='$idtoko'");
        echo'<script>top.document.location.href = "../index.php";</script>';}
            ...
            echo'<script>alert("titik-titik pada kolom set Jam Buka wajib diisi");</script>';
        }
    }
 ...
 ?><br><br><H3>Set Tanggal Buka Kembali</H3><?php if (!empty($status_detail)&&$status_toko=="Closed") {
    ...
    $status="Closed";$info=$_POST['info_libur']; @mysqli_query($conn,"UPDATE infotoko SET status='$status',info_status='$info' WHERE id_toko='$idtoko'");
        echo'<script>top.document.location.href = "../index.php";</script>';}
            ...
            echo'<script>alert("titik-titik pada kolom set tanggal wajib diisi");</script>';
        }
    }
} elseif (isset($_POST['batal_proses'])) {
    echo '<script>alert("Proses Dibatalkan");top.document.location.href = "../dashboard_edit/iframe_edit.php";</script>';
...
<input type="submit" name="resesi" value="Status : Peninjauan Penghapusan" style="background: black;color: white;font-size: 24px;width: 100%;height: 50px;"><?php if (empty($_POST['info_hapus'])) {
 ?><br><br><H3>Saran untuk developer :</H3><?php if (!empty($status_detail)&&$status_toko=="Peninjauan Penghapusan") {
     ?> <textarea name="info_hapus" value="<?php echo $status_detail; ?>" rows="6" cols="40"></textarea> <?php 
 } else { ?><textarea name="info_hapus" placeholder="contoh (rapikan desain, tambah fitur a agar fleksibel, kurangi fitur b karena jarang dipakai, dll.)" rows="6" cols="40"></textarea><?php } ?><br><br><b>- Wajib sertakan kalimat didalam kurung, misal (masalah b....)<br>- Akun akan dinonaktifkan dari panel penjualan<br>- Proses penghapusan akun akan diproses 1x24 jam</b><br><br><input type="submit" value="set" name="hapus_toko" class="btn_submit">&nbsp;<input type="submit" value="Batal" name="batal_proses" class="btn_submit"></form></center></div><div class="cleared"></div><br><br> <?php } else {
...
} else { if ($status_resesi!="in Trouble" || $status_resesi!="Peninjauan Penghapusan") { ?><tr><td colspan="3" style="text-align: center;"><?php echo "Status Toko Saat ini : <H3>".$status_resesi." ".$detail_resesi."</H3>"; ?></td></tr><tr><td><br></td></tr><tr><td><input type="submit" class="btn_submit" name="resesi" value="Ajukan Resesi"></td><td><input type="submit" class="btn_submit" name="hapus_toko" value="Ajukan Hapus Akun Toko"></td>
</tr><?php } }?>
</table></center>
</form>