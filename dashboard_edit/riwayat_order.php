...
$status=$_POST['status'];
echo"<center>";
?><h1 align="center">Daftar Riwayat Order (<?php $result = mysqli_query($conn,"SELECT * FROM pembeli WHERE id_toko='$idtoko' AND status='LUNAS' ORDER BY id_pembeli DESC"); echo mysqli_num_rows($result); ?>) <a href="admin_order.php"><input type="button" value="Order Masuk" style="width:25%;height:50px;"></button></a></h1>';

<?php $result = mysqli_query($conn,"SELECT * FROM pembeli WHERE id_toko='$idtoko' AND status='LUNAS' ORDER BY id_pembeli DESC");
...
while ($row = mysqli_fetch_array($result,MYSQLI_ASSOC)){
  ...
  $status=$row['status'];
         ...
         <tr>
            <td><center><h3>LUNAS</h3><h1>".++$no."</h1></center></td>
