...
import com.example.ubud_souvenir_center.databinding.ActivityMenuInfoBinding
import java.io.File
...
class MenuInfo : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val binding = ActivityMenuInfoBinding.inflate(layoutInflater)
        setContentView(binding.root)

        binding.backmenu3.setOnClickListener {
            val kembalimenu3 = Intent(this@MenuInfo, MenuAdmin1::class.java)
            startActivity(kembalimenu3)
        }

        val tentang = binding.infodeveloper
        if (Build.VERSION.SDK_INT >= 16 && (ContextCompat.checkSelfPermission(this, Manifest.permission.WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED || ContextCompat.checkSelfPermission(this, Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED)) {
            ActivityCompat.requestPermissions(this@MenuInfo, arrayOf(Manifest.permission.WRITE_EXTERNAL_STORAGE, Manifest.permission.CAMERA), 1)
        }
        ...
        if (savedInstanceState == null) {
            tentang.loadUrl("http://ubud-souvenir-center.my.id/panduan.php")
        }
        tentang.webChromeClient = object : WebChromeClient() {
            ...
            override fun onProgressChanged(load : WebView, newProgress : Int) {
                if (newProgress == 100) {
                    val load1 = binding.headerinfo
                    load1.loadUrl("file:///android_asset/blank.html")
                } else {
                    val load2 = binding.headerinfo
        ...
        tentang.webViewClient = object : WebViewClient() {
            ...
            override fun onReceivedError(view: WebView, errorCode: Int, description: String, failingUrl: String) {
                val masalahkoneksi = binding.infodeveloper
    ...
    companion object {
        ...
        private val TAG = MenuInfo::class.java.simpleName
    }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId) {
            R.id.main_menu -> {
                val about = Intent(this@MenuInfo, MenuAdmin1::class.java)
                startActivity(about)
                true
            }
            R.id.halaman_toko -> {
                val about = Intent(this@MenuInfo, InformasiToko2::class.java)
                startActivity(about)
                true
            }
            R.id.halaman_order -> {
                val about = Intent(this@MenuInfo, CekOrder::class.java)
                startActivity(about)
                true
            }
            R.id.halaman_produk -> {
                val about = Intent(this@MenuInfo, KelolaProduk3::class.java)
                startActivity(about)
                true
            }
            R.id.halaman_transaksi -> {
                val about = Intent(this@MenuInfo, KelolaTransaksi4::class.java)
                startActivity(about)
                true
            }
            R.id.halaman_proses_analisa-> {
                val about = Intent(this@MenuInfo, AnalisaJualAmbilData5::class.java)
                startActivity(about)
                true
            }
            R.id.halaman_lihat_analisa -> {
                val about = Intent(this@MenuInfo, LihatAnalisa::class.java)
                startActivity(about)
                true
            }
            R.id.halaman_logout -> {
                val about = Intent(this@MenuInfo, Logout::class.java)
                startActivity(about)
                true
            }
            R.id.halaman_riwayat_order -> {
                val about = Intent(this@MenuInfo, RiwayatOrder::class.java)
                startActivity(about)
                true
            }
            R.id.halaman_kontak -> {
                val about = Intent(this@MenuInfo, MenuInfo::class.java)
                startActivity(about)
                true
            }
            R.id.tutup_aplikasi -> {
                android.app.AlertDialog.Builder(this).setIcon(android.R.drawable.ic_dialog_alert)
                    ...
                    .setPositiveButton("YA"
                        ...
                        finishAffinity()
                        Toast.makeText(this@MenuInfo, "Aplikasi Ditutup", Toast.LENGTH_SHORT).show()
