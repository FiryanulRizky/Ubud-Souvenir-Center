...
$cek2=mysqli_query($conn,"SELECT SUM(Januari+Februari+Maret+April+Mei+Juni+Juli+Agustus+September+Oktober+November+Desember) AS total FROM pendapatan WHERE id_toko = '$idtoko'");
$ambil_cek=mysqli_fetch_array($cek2);
$cek_pendapatan=$ambil_cek['total'];
  if(mysqli_num_rows($cek)>0 && $cek_pendapatan>0) {
...
<html>
  ...
  <body>
    ...
    <br><center><a href="linechart_jumlah_jual.php"><H1>RIWAYAT PENDAPATAN</H1></a>
    <table width="40%" height="40" style="text-align: center; border-bottom: 1px solid red; font-weight: bold; font-size: 21px;"><tr><td><?php echo "cpm : ".format_currency($cpm);?></td><td><?php echo "total : ".format_currency($pendapatan_bersih);?></td></tr></table>
    </center>
    <div class="container">
