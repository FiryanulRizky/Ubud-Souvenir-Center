...
if (isset($_POST['kata_kunci'])) {
            ...
            $result=mysqli_query($conn,"SELECT * FROM itemc3 WHERE $kolom like '%".$kata_kunci."%' AND persen_support >='$nRule' AND id_toko='$idtoko'");if (mysqli_num_rows($result)>0) {
            	echo "Data Tabel 2 ini telah disortir berdasarkan $kolom dengan kata kunci $kata_kunci";?>
            	<form method="POST" action="pola_3item.php"><button type="submit" name="reset">Hapus Filter</button></form><?php
            } else {
            	echo "Data $kolom dengan kata kunci $kata_kunci kosong";?>
            <form method="POST" action="pola_3item.php"><button type="submit" name="reset" >Hapus Filter</button></form>
            <?php
            }
        }else {
        ...
        <?php
            ...
            $MerkItem3=mysqli_query($conn,"SELECT * FROM item WHERE kditem='$C2kditem3' AND id_toko='$idtoko'");
