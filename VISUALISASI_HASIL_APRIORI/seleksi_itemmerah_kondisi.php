...
<form action="<?php echo $_SERVER["PHP_SELF"];?>" method="post">
	<div class="form-group">
      	<div class="form-group">
      	<label for="sel1"><h3 style="color: red;">KODE MERAH</h3> (Penjualan 75% keatas)</label><br>
...
if (isset($_POST['kata_kunci'])) {
            ...
            $result=mysqli_query($conn,"SELECT * FROM itemc2 WHERE id_toko='$idtoko' AND $kolom like '%".$kata_kunci."%' AND persen_support >='$nRule'");if (mysqli_num_rows($result)>0) {
            	...
            	<form method="POST" action="seleksi_itemmerah.php"><button type="submit" name="reset">Hapus Filter</button></form><?php
            } else {
            	echo "Data $kolom dengan kata kunci $kata_kunci kosong";?>
            <form method="POST" action="seleksi_itemmerah.php"><button type="submit" name="reset" >Hapus Filter</button></form>
            <?php
            }
        }else {
		...
		while($rowC2 = mysqli_fetch_array($result,MYSQLI_ASSOC))
		{
			...
			$DataMerkItem1=mysqli_fetch_array($MerkItem1,MYSQLI_ASSOC);?> <div id="circle_2kiri"><div class="circle_2itemkiri"><?php echo "[".$DataMerkItem1['kditem']."]"."<br><img src='../gambar/produk/".$DataMerkItem1['gambar_item']."'><br>".$DataMerkItem1['merk'].",";?></div><div class="circle_merah"><H2>K</H2><?php echo "<h3>membeli $persensupport</h3>";?></div><?php 
			...
			if ($persensupport2 >= 75) {
				...
				?><br><br><br><hr></div>
				<div class="circle_merah2"><H2>K</H2><?php echo "<h3>membeli $persensupport2</h3>";?></div>
				<?php 
				$MerkItem1=mysqli_query($conn,"SELECT * FROM item WHERE id_toko='$idtoko' AND kditem='$C2kditem1' ");
