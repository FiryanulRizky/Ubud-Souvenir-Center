...
while($tangkap_1=mysqli_fetch_array($data_trans_1,MYSQLI_ASSOC)) {
    ...
    $data_merk_1=mysqli_query($conn,"SELECT item.kditem,item.merk,item.gambar_item,item.harga FROM transaksi,item WHERE item.kditem=transaksi.kditem AND transaksi.id_toko=item.id_toko AND transaksi.id_toko='$idtoko' AND transaksi.kdtransaksi='$kdtransaksi' ");
