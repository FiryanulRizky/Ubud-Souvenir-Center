<html>
    <head>
        <title>Tinjau Hapus Super Admin</title>
...
$cek_toko=mysqli_query($conn,"SELECT * FROM infotoko WHERE status='Peninjauan Penghapusan' ORDER BY id_toko ASC");
echo"<div id='bgkonten'>";
if (mysqli_num_rows($cek_toko)>0) {
echo "<a href='../header_session/konfirmasi_logout_sp.php' style='color:red;font-weight:bold;'>Anda Login Sebagai ".$nama_sp." (Super Admin)</a><h2><center>List Toko Dalam Peninjauan Penghapusan</center></h2>";
  ...
  <div class="row">
    <div class="col-xs-12">
      <div class="panel-group">
        ...
        if ($status_detail=="") {
            echo"<center><form method='post' action='../crud/hapus_toko/hapus_toko.php?id=".$idtoko."'><input type='submit' name='tinjau_hapus' value='Status Toko : ".$status_toko."' style='background:black;color:white; padding:5px; font-size:21px;'></form></center><br>";
        } else {
        echo"<center><form method='post' action='../crud/hapus_toko/hapus_toko.php?id=".$idtoko."'><input type='submit' name='tinjau_hapus' value='Status Toko : ".$status_toko." Pesan : ".$status_detail."' style='background:black; color:white; padding:5px; font-size:21px;'></form></center><br>";
        }
...
<?php } } else {
  echo'<center><H3>BELUM ADA PERMINTAAN HAPUS AKUN TOKO</H3></center>';
} include"../header_session/footer_sp.php"; echo"</div>";?>