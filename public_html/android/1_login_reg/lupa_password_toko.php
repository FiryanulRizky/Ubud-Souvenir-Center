...
if ($_POST['act']=="login") {
  ...
  if($err>0){ // *** if submit error
   ...
   if ($_POST['password_baru']==$_POST['password_baru_konfirm']){
    ...
    echo'<script>alert("Password pemilik baru Berhasil dibuat");window.location ="login_reg.php";</script>'; } if($_GET['proses']=="kunci_akses") {
       mysqli_query($conn,"UPDATE infotoko SET kunci_akses='".md5($_POST['password_baru'])."' WHERE id_toko='".$_POST['pilih_toko']."'");
    echo'<script>alert("Kunci akses baru Berhasil dibuat");window.location ="login_reg.php";</script>'; }
   } else {
...
<option value="password_pemilik">Password Pemilik Baru</option>
<option value="kunci_akses">Kunci Akses Baru</option>
</select>
<br><br>
<input type="submit" value="Pilih (Klik 2x)" class="btn_submit"></form>
<form method="post"><br>
<input class="tombol_login" type="submit" value="KEMBALI" name="batal_admin"><br><br>
</form>
</div>
<br><br><br><br><br><br><br><br><br><br><br><br><br><br><br>
<?php if(isset($_POST['batal_admin'])) {
