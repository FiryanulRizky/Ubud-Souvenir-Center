...
if ($_POST['act']=="register"){
    ...
    if($cek > 0) {
      ...
      if($ambil_akses['kunci_akses']==md5($_POST['kunci_akses'])) {    
        $sql=mysqli_query($conn,"INSERT INTO admin_web (id_toko,username,password,nama,telepon,jabatan) VALUES ('$pilihtoko','$username','$password','$nama','$no_hp','$admin')"); 
        echo'<script>alert("Akun berhasil dibuat, silahkan login");window.location="login_reg.php";</script>';   
      } else {
