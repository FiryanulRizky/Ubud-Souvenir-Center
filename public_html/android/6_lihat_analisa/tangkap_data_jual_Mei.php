...
<div id="bgkonten">
<br>
<center><H2>PROSES GRAFIK LINE REKAM JEJAK TRANSAKSI</H2><br><table><tr><td style="border: 1px solid; background: lightgreen;"><a href="iframe_graph.php"><H3>MENU VISUALISASI</H3></a></td><td>&nbsp</td><td style="border: 1px solid; background: orange;"><a href="tangkap_data_jual_Apr.php"><H3>KEMBALI</H3></a></td><td>&nbsp</td><td><form method='post' action='tangkap_data_jual_Jun.php'><input type='submit' class='btn_submit' name='pindah' value='SELANJUTNYA' style="width: 100%;"></form></td></tr></table></center><br>
...
echo"<H1 style='color:blue; text-shadow: 1px 1px black;'>Penjualan Bulan Mei<hr>";

$data_trans_1=mysqli_query($conn,"SELECT * FROM transaksi WHERE id_toko='$idtoko' AND MONTH(tgltransaksi)=5 GROUP BY kdtransaksi");
$data_trans_1_s=mysqli_query($conn,"SELECT * FROM transaksi WHERE id_toko='$idtoko' AND MONTH(tgltransaksi)=5");
...
$pendapatan_bulanan=mysqli_query($conn,"SELECT SUM(Mei) AS total FROM pendapatan WHERE id_toko='$idtoko'");
    ...
    function cari($data, $data2)
    {
        ...
        {    
            ...
            $selectors = htmlentities(implode(',', $produk_ar));
                ...
                if ($_POST['act']=="line") {
                $cek_isi_db=mysqli_query($conn,"SELECT * FROM graph WHERE id_toko='$idtoko' AND produk='$baris' AND Mei='$laku'");
                $cek_kd=mysqli_fetch_array($cek_isi_db);
                $ck1=$cek_kd['kditem'];
                $cek_jumlah_db=mysqli_num_rows($cek_isi_db);
                if ($cek_jumlah_db>0 && $ck1==$kditem) {

                    echo'<script>alert("DATA SUDAH ADA");window.location="tangkap_data_jual_Mei.php"</script>';

                } else {

                    @mysqli_query($conn,"UPDATE graph SET Mei= Mei + '$laku' WHERE id_toko='$idtoko' AND kditem='$kditem' AND produk='$baris'");
                    @mysqli_query($conn,"UPDATE pendapatan SET Mei= Mei + '$total' WHERE id_toko='$idtoko' AND kditem='$kditem' AND produk='$baris'");

                        echo'<script>alert("Data Berhasil di Update");window.location ="tangkap_data_jual_Mei.php";</script>';
                    }
                }   
            }
        }
