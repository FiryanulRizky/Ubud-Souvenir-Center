...
?>
	...
	$strRule=mysqli_query($conn,"SELECT * FROM rule WHERE kdrule='R3' AND id_toko='$idtoko'");
...
<br><form action="<?php echo $_SERVER["PHP_SELF"];?>" method="post">
    ...
    <table>
   	...
   	<td><div class="form-group">
        <input type="submit" class="btn btn-info" value="Pilih" style="font-size:24px;font-weight:bold;"></form></div></td>
    <td>&nbsp;&nbsp;&nbsp;&nbsp;</td><td><div class="form-group">
    ...
    if (!preg_match("/^[1-9][0-9]*$/",$_POST['kata_kunci'])) {
        echo'<script>alert("Harga hanya menerima input angka");window.location="'.$_SERVER['PHP_SELF'].'";</script>'; } else {
            ...
            if ($_POST['kolom']=="kditem1")
            {
            	...
            	<form method="POST" action="seleksi_3item.php"><input type="submit" name="reset" value="Hapus Filter" style="font-size:24px;font-weight:bold;background:red;color:white;padding:10px;"></form></center><br><?php
            } else {
            	echo "<center><b style='font-size:36px;'>ID Item Kiri (BR$kata_kunci) kosong</b>";?>
            <form method="POST" action="seleksi_3item.php"><input type="submit" name="reset" value="Hapus Filter" style="font-size:24px;font-weight:bold;padding:10px;background:red;color:white;"></form></center>
            <?php
            }
            }else if ($_POST['kolom']=="kditem2"){
                $kolom="kditem2";
            ...
            $result=mysqli_query($conn,"SELECT * FROM itemc3 WHERE persen_support >='$nRule' AND lift_ratio>=1 AND id_toko='$idtoko'");
        }
        ...
        <?php
            ...
            $MerkItem1=mysqli_query($conn,"SELECT * FROM item WHERE kditem='$C2kditem1' AND id_toko='$idtoko'");
            ...
            $MerkItem2=mysqli_query($conn,"SELECT * FROM item WHERE kditem='$C2kditem2' AND id_toko='$idtoko'");?> <div class="circle_3itemtengah"><?php
            ...
            //menampilkan data kditem2 c2
            $kditem=$C2kditem1;
            $query_T=mysqli_query($conn,"SELECT * FROM transaksi WHERE kditem='$kditem' AND id_toko='$idtoko'");
            ...
            <?php }
            $MerkItem3=mysqli_query($conn,"SELECT * FROM item WHERE kditem='$C2kditem3' AND id_toko='$idtoko'");?> <div class="circle_3itemkanan"><?php
            ...
            $MerkItem3=mysqli_query($conn,"SELECT * FROM item WHERE kditem='$C2kditem3' ");
            ...
            ?></div><div class="circle_3dotkanan"><b>Sebaliknya</b></div></div><br><?php } ?>
</body>
</html>