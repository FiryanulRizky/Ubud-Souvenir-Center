...
<form action="<?php echo $_SERVER["PHP_SELF"];?>" method="post">
	<div class="form-group">
      	<div class="form-group">
      	<label for="sel1"><h3 style="color: gold;font-size:32px;">KODE KUNING</h3> <b style="font-size:24px;">(Penjualan Antara 25% s.d 50%)</b></label><br>
    ...
    <table>
   	...
   	<td><div class="form-group">
        ...
        <form action="seleksi_itemkuning.php" method="post">
    ...
    if (!preg_match("/^[1-9][0-9]*$/",$_POST['kata_kunci'])) {
        echo'<script>alert("Harga hanya menerima input angka");window.location="'.$_SERVER['PHP_SELF'].'";</script>'; } else {
            ...
            if ($_POST['kolom']=="kditem1")
            {
            	...
            	<form method="POST" action="seleksi_itemkuning.php"><input type="submit" name="reset" value="Hapus Filter" style="font-size:24px;font-weight:bold;background:red;color:white;padding:10px;"></form></center><br><?php
            } else {
            	echo "<center><b style='font-size:36px;'>ID Item Kiri (BR$kata_kunci) kosong</b>";?>
            <form method="POST" action="seleksi_itemkuning.php"><input type="submit" name="reset" value="Hapus Filter" style="font-size:24px;font-weight:bold;padding:10px;background:red;color:white;"></form></center>
            <?php
            }
            }else if ($_POST['kolom']=="kditem2"){
                $kolom="kditem2";
		...
		while($rowC2 = mysqli_fetch_array($result,MYSQLI_ASSOC))
		{
			...
			$DataMerkItem1=mysqli_fetch_array($MerkItem1,MYSQLI_ASSOC);?> <div id="circle_2kiri"><div class="circle_2itemkiri"><?php echo "[".$DataMerkItem1['kditem']."]"."<br><img src='../../gambar/produk/".$DataMerkItem1['gambar_item']."'><br>".$DataMerkItem1['merk'].",";?></div><div class="circle_kuning"><H2>K</H2><?php echo "<h3>membeli $persensupport</h3>";?></div><?php 
			...
			if ($persensupport2 > 25 && $persensupport2 <=50 ) {
				...
				?><br><br><br><hr></div>
				<div class="circle_kuning2"><H2>K</H2><?php echo "<h3>membeli $persensupport2</h3>";?></div>
				<?php 
				$MerkItem1=mysqli_query($conn,"SELECT * FROM item WHERE id_toko='$idtoko' AND kditem='$C2kditem1' ");
