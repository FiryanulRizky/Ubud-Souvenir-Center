...
echo"<H1 style='color:blue; text-shadow: 1px 1px black;'>Penjualan Bulan Desember<hr>";

$data_trans_1=mysqli_query($conn,"SELECT * FROM transaksi WHERE id_toko='$idtoko' AND MONTH(tgltransaksi)=12 GROUP BY kdtransaksi");
$data_trans_1_s=mysqli_query($conn,"SELECT * FROM transaksi WHERE id_toko='$idtoko' AND MONTH(tgltransaksi)=12");
...
$pendapatan_bulanan=mysqli_query($conn,"SELECT SUM(Desember) AS total FROM pendapatan WHERE id_toko='$idtoko'");
    ...
    function cari($data, $data2)
    {
        ...
        {    
            ...
            $selectors = htmlentities(implode(',', $produk_ar));
                ...
                if ($_POST['act']=="line") {
                $cek_isi_db=mysqli_query($conn,"SELECT * FROM graph WHERE id_toko='$idtoko' AND produk='$baris' AND Desember='$laku'");
                $cek_kd=mysqli_fetch_array($cek_isi_db);
                $ck1=$cek_kd['kditem'];
                $cek_jumlah_db=mysqli_num_rows($cek_isi_db);
                if ($cek_jumlah_db>0 && $ck1==$kditem) {

                    echo'<script>alert("DATA SUDAH ADA");window.location="tangkap_data_jual_Des.php"</script>';

                } else {

                    @mysqli_query($conn,"UPDATE graph SET Desember= Desember + '$laku' WHERE id_toko='$idtoko' AND kditem='$kditem' AND produk='$baris'");
                    @mysqli_query($conn,"UPDATE pendapatan SET Desember= Desember + '$total' WHERE id_toko='$idtoko' AND kditem='$kditem' AND produk='$baris'");

                        echo'<script>alert("Data Berhasil di Update");window.location ="tangkap_data_jual_Des.php";</script>';
                    }
                }   
            }
        }
