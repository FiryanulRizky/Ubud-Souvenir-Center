...
$jml_jual=mysqli_query($conn,"SELECT SUM(Januari+Februari+Maret+April+Mei+Juni+Juli+Agustus+September+Oktober+November+Desember) AS total FROM graph WHERE id_toko = '$idtoko'");
$ambil_jual=mysqli_fetch_array($jml_jual);
$cek2=$ambil_jual['total'];
...
<html>
  ...
  <body style="background:#DFE6F0;">
    <center><a href="iframe_graph.php"><H3>RIWAYAT ITEM TERJUAL</H3></a></center>
    <?php ob_start(); include"tangkap_data_cpm.php"; ob_end_clean(); ?>
    <center></center><table width="100%" height="40" style="text-align: center; border-bottom: 1px solid red; font-weight: bold; font-size: 14px;"><tr><td><?php echo "terjual/bulan : ".ceil($rerata_item);?></td><td><?php echo "total terjual: ".$jumlah_item;?></td></tr></table>
    </center>
    <div class="container">
