...
if (isset($_POST['kata_kunci'])) {
            ...
            $result=mysqli_query($conn,"SELECT * FROM itemc3 WHERE id_toko='$idtoko' AND $kolom like '%".$kata_kunci."%' AND persen_support >='$nRule'");if (mysqli_num_rows($result)>0) {
            	...
            	<form method="POST" action="seleksi_itemkuning2.php"><button type="submit" name="reset">Hapus Filter</button></form><?php
            } else {
            	echo "Data $kolom dengan kata kunci $kata_kunci kosong";?>
            <form method="POST" action="seleksi_itemkuning2.php"><button type="submit" name="reset" >Hapus Filter</button></form>
            <?php
            }
        }else {
