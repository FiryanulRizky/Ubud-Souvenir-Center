...
<input type="submit" name="hapus_toko" value="Status : in Trouble" style="background: red;color: white;font-size: 24px;width: 100%;height: 50px;"><?php if (empty($_POST['info_resesi'])) {
 ...
 } else { ?><textarea name="info_resesi" placeholder="contoh : (masalah internal toko (masalah keuangan), perlu reksadana, perlu inkubator bisnis (diskusi dengan pegiat bisnis digital), tambah fitur a karena.., kurangi fitur b karena..., bug di bagian order, desain sulit dipahami, dll.)" rows="8" cols="40"></textarea><?php } ?><br><br><b>- Wajib sertakan kalimat didalam kurung, misal (masalah b....)<br>- Fitur ini akan menonaktifkan akun toko dari panel penjualan</b><br><br><h3 style="color : red;">- 1 pesan = membantu anda maju menuju ukm digital</h3><input type="submit" value="set" name="resesi" class="btn_submit">&nbsp;<input type="submit" value="Batal" name="batal_proses" class="btn_submit"></form></center></div><div class="cleared"></div><br><br> <?php } else {
...
} else { if ($status_resesi!="in Trouble" || $status_resesi!="Peninjauan Penghapusan") { ?><tr><td colspan="3" style="text-align: center;"><?php echo "Status Toko Saat ini : <H3>".$status_resesi." ".$detail_resesi."</H3>"; ?></td></tr><tr><td><input type="submit" class="btn_submit" style="width:100%;" name="resesi" value="Ajukan Resesi"></td><td>&nbsp;&nbsp;&nbsp;&nbsp;</td><td><input type="submit" style="width:100%;" class="btn_submit" name="hapus_toko" value="Ajukan Hapus Akun Toko"></td>
</tr><?php } }?>
</table></center>
</form>